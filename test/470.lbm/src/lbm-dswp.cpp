...
struct LBM_performStreamCollide_packet 
{
	int i;
  double ux, uy, uz, u2, rho;
};

struct LBM_performStreamCollide_worker_args {
  double *&dstGrid;
  double *&srcGrid;
	pc_queue<LBM_performStreamCollide_packet> &queue;
};

void LBM_performStreamCollide_worker(
		LBM_performStreamCollide_worker_args *args) {
	double *&dstGrid = args->dstGrid;
	double *&srcGrid = args->srcGrid;
	auto &queue = args->queue;
  while (!queue.end_reached()) {
		const LBM_performStreamCollide_packet &p = queue.consume();
		int i = p.i;
		double ux = p.ux, uy = p.uy, uz = p.uz, u2 = p.u2, rho = p.rho;
...
void LBM_performStreamCollide( LBM_Grid srcGrid, LBM_Grid dstGrid ) {
  ...
  pc_queue<LBM_performStreamCollide_packet> queue;
	LBM_performStreamCollide_worker_args args{dstGrid, srcGrid, queue};
  std::thread c(LBM_performStreamCollide_worker, &args);
	...
	SWEEP_START( 0, 0, 0, 0, 0, SIZE_Z )
		...
		queue.produce({i, ux, uy, uz, u2, rho});
	SWEEP_END
	queue.produce_end();
	c.join();
}
